...
| # | 文件 | 改动 |
|---|------|------|
| 1 | `api/questApi.js` | 新增 `ensureTaskBranch(questId)` → `POST /quests/{questId}/task-branch` |
| 2 | `pages/quests/QuestDetailPage.vue` | 把路由真实 `questId` 透传给 `QuestDetail` |
| 3 | `components/QuestDetail.vue` | 接取动作改为真实调用 `questApi.assign()`（数字 questId 时），展示返回的 `taskBranch`；分支为空时调 `ensureTaskBranch` 幂等补建；新增"任务分支"展示项 |
| 4 | `components/Workbench.vue` | 创建分支动作在 task 携带真实 `questId` 时调真实端点，Gitea 返回分支名后覆盖本地占位；mock 演示任务保留本地命名 |

> 说明：详情页接取是真实"创建 active Assignment + task branch"的数据点，已展示真实分支名。工作台目前仍以 mock `data/workbench.js` 驱动任务列表（无真实 questId），其"创建分支"按钮已接好真实端点，待 #15 工作台 API 联调把真实接取记录喂入后即可端到端展示。
